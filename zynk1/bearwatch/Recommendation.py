...
def get_tickers_from_finviz(sector,market_cap):
    ...
    if market_cap < 1000000000: #all market cap values subject to change
        size = "small"
    ...
    querystring = {
        "order": "marketcap",
        "desc": "true",
        "filters": {f"cap_{size}":"1", f"sec_{sector}":"1"}  # Using sector filter and market cap filter
    ...
    if sector != 'N/A':
        ...
        if tickers:
            ...
            if recommended_stocks:
                print(f"Stocks similar to {input_ticker}: {recommended_stocks}")
        else:
